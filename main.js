...
app.get("/", function(req, res) {
	...
	for (var i = 0; i < thread_names.length; i++) {
		...
		app.post("/" + current_thread + "/new_post", urlencoded_parser, function(req, res) {
			threads[current_thread].push([cleanInput(req.body.username), cleanInput(req.body.post)]);
...
app.post("/new_thread", urlencoded_parser, function(req, res) {
	console.log(encodeURIComponent(cleanInput(req.body.thread_name)));
	threads[encodeURIComponent(cleanInput(req.body.thread_name))] = [[cleanInput(req.body.username), cleanInput(req.body.thread_name)]];
